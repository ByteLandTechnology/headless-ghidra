...
if (
  TARGET_FUNCS.length === 0 ||
  (TARGET_FUNCS.length === 1 && TARGET_FUNCS[0] === "")
) {
...
for (const funcName of TARGET_FUNCS) {
  ...
  Interceptor.attach(addr, {
    onEnter: function (args) {
      ...
      for (let i = 0; i < 4; i++) {
        try {
          ...
          else if (arg.toInt32 && arg.toInt32() > 0 && arg.toInt32() < 0x10000)
            typeHint = "int";
    ...
    onLeave: function (retval) {
      ...
      let retHint = "unknown";
      if (retval.toString().startsWith("0x") && !retval.isNull())
        retHint = "pointer";
      else if (retval.toInt32)
        retHint = retval.toInt32() === 0 ? "null" : "int";
      ...
      console.log(
        JSON.stringify({
          ...
          inferred_signature:
            ...
            funcName +
            "(" +
            info.argTypes.map((t, i) => t + " arg" + i).join(", ") +
            ")",
