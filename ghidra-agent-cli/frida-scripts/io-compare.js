...
{
    const originalLog = JSON.parse(new TextDecoder().decode(
        readFileSync(env.IOLOG_ORIG || '/tmp/iolog_original.json')
    ));
    const reconstructedLog = JSON.parse(new TextDecoder().decode(
    ...
    for (let i = 0; i < Math.min(originalLog.length, reconstructedLog.length); i++) {
        const orig = originalLog[i];
        const recon = reconstructedLog[i];

        if (orig.type !== recon.type || orig.name !== recon.name) {
            console.log(JSON.stringify({ match: false, index: i, reason: 'type/name mismatch', orig, recon }));
            mismatches++;
            continue;
        }
        ...
        if (argsMatch && retMatch) {
            ...
            console.log(JSON.stringify({
                match: false, index: i, args_match: argsMatch, ret_match: retMatch,
                orig_args: orig.args, recon_args: recon.args,
                orig_ret: orig.return_value, recon_ret: recon.return_value
            }));
            mismatches++;
        }
    }

    console.log(JSON.stringify({ summary: true, matches, mismatches, total: originalLog.length }));
}
